...
public:
    ...
    void in_lobby_screen(sf::RenderWindow &window)
    {
        ...
        for (int i = curr_lobby.position_in_game; i >= 0; i--)
        {
            ...
            if (curr_lobby.players[form_first].has_up_cards)
                window.draw(spriteShown);
            if (curr_lobby.players[form_first].has_down_cards)
        ...
        for (std::vector<Player>::size_type i = curr_lobby.position_in_game + 1; i < curr_lobby.players.size(); ++i)
        {
            ...
            window.draw(playerText);
            if (curr_lobby.players[i].has_up_cards)
                window.draw(spriteShown);
            if (curr_lobby.players[i].has_down_cards)
                window.draw(spriteHidden);
            from_last++;
        }
    ...
    void set_holder_pos_and_name(int &fd)
    {
        ...
        for (std::vector<Player>::size_type i = 0; i < curr_lobby.players.size(); ++i)
        {
            ...
            {
                curr_lobby.totem_holder_pos = static_cast<std::vector<Player>::size_type>(curr_lobby.position_in_game) >= i ? curr_lobby.position_in_game - i : 7 - from_last;

                curr_lobby.totem_holder_name = curr_lobby.players[i].username;
                return;
            }
            if (static_cast<std::vector<Player>::size_type>(curr_lobby.position_in_game) < i)
                from_last++;
        }
    }
